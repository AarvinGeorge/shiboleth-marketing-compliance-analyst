...
<script>
(function () {
  var shibolethQuote = %(quote_json)s;
  var shibolethQuoteRegexSrc = %(regex_json)s;
  var CLS = "shiboleth-evidence-mark";
  function attemptExact(cb) {
    var found = false;
    new Mark(document.body).mark(shibolethQuote, {
  ...
  function attemptRegex(cb) {
    var found = false;
    new Mark(document.body).markRegExp(new RegExp(shibolethQuoteRegexSrc, "g"), {
      className: CLS,
      each: function () { found = true; },
      done: function () { cb(found); }
    });
  ...
  function report(found) {
    try {
      parent.postMessage({ type: "shiboleth-preview", found: found }, "*");
    } catch (e) {}
  }
