"""
meta:
  purpose: FastAPI application factory for the Adlign API. M0 scope: app
...
from adlign.config import Settings, load_settings

logger = logging.getLogger("adlign")
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Adlign env verified:\n%s", settings.masked_echo())
...
def create_app() -> FastAPI:
    app = FastAPI(title="Adlign API", version="0.1.0", lifespan=lifespan)
    ...
    from adlign.api.routes.flags import router as flags_router
    from adlign.api.routes.metrics import router as metrics_router
    from adlign.api.routes.preview import router as preview_router
    from adlign.api.routes.products import router as products_router
    from adlign.api.routes.runs import router as runs_router
    from adlign.api.routes.scorecard import router as scorecard_router
