...
meta:
  ...
  contract: seed(session) upserts by deterministic natural ids (SC-01, R-0x,
            ...
            Run: uv run python -m adlign.db.seed
  deps: sqlalchemy, adlign.db.models, adlign.db.seed_rules (generated).
"""
...
async def seed(session: AsyncSession) -> dict[str, int]:
    await _upsert(session, Scorecard, {
        "id": SCORECARD_ID, "name": "Bank-partner marketing scorecard", "version": 1,
    })
