...
from adlign.db.models import Cluster, Flag, Material, Product, Property, Run
...
async def product_detail(product_id: str, request: Request) -> dict:
    async with request.app.state.session_factory() as session:
        ...
        if latest:
            ...
            from adlign.api.routes.metrics import (needs_review_flag_ids,
...
async def extract_properties_route(text: str) -> list[dict]:
    ...
    from adlign.services.ingestion.extract import extract_properties
